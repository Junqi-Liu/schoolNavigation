...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
	    ...
	    <script type="text/javascript" src="<%=basepath%>/static/js/pointLookList.js"></script>

	</head>
	<body style="background: #e1e9eb;">
	<input type="hidden" name="basepath" value="${basepath}"/>
		<form action="selectByDescp" id="mainForm" method="post">
			...
			<div class="right">
				<div class="rightCont">
					...
					<table class="tab1">
						<tbody>
							<tr>
								...
								<td>
									<input name="pointdesc" id="pointdesc" value="" class="allInput" type="text"/>
								</td>
	                            <td style="text-align: right;" width="160">
	                            	<input class="tabSub" value="查询" onclick="" type="submit"/>&nbsp;&nbsp;&nbsp;&nbsp;
	                            	<t:auth url="/ad/addInit">
									<input class="tabSub" value="添加" onclick="location.href='${basepath}/pointToAdd'" type="button" />
								</t:auth>
	                            </td>
	       					</tr>
						</tbody>
						...
						<table class="tab2" width="100%">
							<tbody>
								...
								<tr>
									...
									<td>${point.type}</td>
									<td>
										<c:if test="${point.state=='U'}">正常</c:if>
										<c:if test="${point.state=='E'}">已删除</c:if>
										<c:if test="${point.state=='W'}">确认中</c:if>
									</td>
								</tr>
							</c:forEach>			
							</tbody>
					...
					<div class="page fix">
							<a href="getPointLookList?pageIndex=1" class="first">首页</a>
						<c:if test="${pointPageList.hasPreviousPage}">
							<a href="getPointLookList?pageIndex=${pointPageList.getPageNum()-1}" class="pre">上一页</a>
						</c:if>
						当前第<span>${pointPageList.pageNum}/${pointPageList.pages}</span>页
						<c:if test="${pointPageList.hasNextPage}">
							<a href="getPointLookList?pageIndex=${pointPageList.getPageNum()+1}" class="next">下一页</a>
						</c:if>
							<a href="getPointLookList?pageIndex=${pointPageList.getPages()}" class="last">末页</a>
